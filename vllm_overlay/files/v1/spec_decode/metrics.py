...
class SpecDecodingStats:
    ...
    def observe_draft(self, num_draft_tokens: int, num_accepted_tokens: int):
        ...
        if num_draft_tokens > 0 and num_accepted_tokens == num_draft_tokens:
            self.num_bonus_activations += 1
...
class SpecDecodingLogging:
    ...
    def reset(self):
        ...
        self.num_bonus_activations: list[int] = []
    ...
    def observe(self, spec_decoding_stats: SpecDecodingStats):
        ...
        self.num_bonus_activations.append(
            spec_decoding_stats.num_bonus_activations
        )
    ...
    def log(self, log_fn=logger.info):
        ...
        num_bonus_activations = np.sum(self.num_bonus_activations)
        draft_throughput = 0
...
class SpecDecodingProm:
    ...
    def __init__(
        ...
        self.relaxed_bonus_token_policy = getattr(
            speculative_config, "relaxed_bonus_token_policy", "target_p"
        )
        ...
        counter_bonus_activations = self._counter_cls(
            name="vllm:spec_decode_num_bonus_activations",
            documentation="Number of times a speculative bonus token is emitted.",
            labelnames=labelnames,
        )
        self.counter_spec_decode_num_bonus_activations = create_metric_per_engine(
            counter_bonus_activations, per_engine_labelvalues
        )

        counter_target_bonus_activations = self._counter_cls(
            name="vllm:spec_decode_num_target_bonus_activations",
            documentation="Number of target-distribution bonus token activations.",
            labelnames=labelnames,
        )
        self.counter_spec_decode_num_target_bonus_activations = (
            create_metric_per_engine(
                counter_target_bonus_activations, per_engine_labelvalues
            )
        )

        counter_relaxed_bonus_activations = self._counter_cls(
            name="vllm:spec_decode_num_relaxed_bonus_activations",
            documentation="Number of relaxed-distribution bonus token activations.",
            labelnames=labelnames,
        )
        self.counter_spec_decode_num_relaxed_bonus_activations = (
            create_metric_per_engine(
                counter_relaxed_bonus_activations, per_engine_labelvalues
            )
        )
    ...
    def observe(self, spec_decoding_stats: SpecDecodingStats, engine_idx: int = 0):
        ...
        self.counter_spec_decode_num_bonus_activations[engine_idx].inc(
            spec_decoding_stats.num_bonus_activations
        )
        if self.relaxed_bonus_token_policy == "relaxed_T_qp":
            self.counter_spec_decode_num_relaxed_bonus_activations[engine_idx].inc(
                spec_decoding_stats.num_bonus_activations
            )
        else:
            self.counter_spec_decode_num_target_bonus_activations[engine_idx].inc(
                spec_decoding_stats.num_bonus_activations
            )
