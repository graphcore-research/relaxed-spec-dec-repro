...
SCD_DEBUG_LOG_ALL_ENV = "SPEC_DEC_DEBUG_SCD_NAN_LOG_ALL"
...
def _scd_debug_log_all() -> bool:
    value = os.getenv(SCD_DEBUG_LOG_ALL_ENV)
    return value is not None and value.lower() not in ("", "0", "false", "no")
...
def _scd_debug_tensor_stats(
    ...
    *,
    ...
    with torch.no_grad():
        ...
        if first_nan_row is not None and data.ndim == 2 and draft_token_ids is not None:
            token_ids = draft_token_ids.detach().reshape(-1)
            if first_nan_row < token_ids.numel():
                token_id = int(token_ids[first_nan_row].item())
                stats["selected_token_id"] = token_id
                if 0 <= token_id < data.shape[1]:
                    value = data[first_nan_row, token_id]
...
def _log_mtp_draft_debug_tensors(
    stage: str,
    ...
    *,
    ...
    if not _scd_debug_enabled():
        return
    stats = [
        _scd_debug_tensor_stats(name, tensor, draft_token_ids=draft_token_ids)
        for name, tensor in tensors.items()
    ]
    has_nan = any(int(item.get("nan_count", 0)) > 0 for item in stats)
    if not has_nan and not _scd_debug_log_all():
        return
    _write_scd_debug_event(
        {
            "stage": stage,
            "extra": extra or {},
            "tensors": stats,
...
class SpecDecodeBaseProposer:
    def __init__(
        ...
        self.last_draft_entropies: torch.Tensor | None = None
        self.last_draft_rule_logits: torch.Tensor | None = None
    ...
    def _uses_stochastic_draft_sampling(self) -> bool:
        return (
            self.method in ("draft_model", "mtp")
            and self.speculative_config.draft_sampling_method == "stochastic"
        )
    ...
    def _sample_draft_token(
        ...
        """Sample one draft position.

        Shapes: token ids [B], draft probs/logits [B, V], entropies [B].
        """
        if not self._uses_stochastic_draft_sampling():
            return self._greedy_sample(hidden_states), None, None, None

        logits = self.model.compute_logits(hidden_states)
        token_ids, probs, rule_logits, entropies = compute_probs_and_sample_next_token(
            logits, sampling_metadata
        )
        return token_ids, probs, rule_logits, entropies
    ...
    def propose_one(
        ...
        """Run one drafter step and return sampled ids, q probs, and logits.

        This is used by relaxed q-bonus after target verification. It mirrors
        the first pass of `propose()` but intentionally stops before generating
        the usual multi-token draft block.

        Shapes: returned token ids [B], returned probs/logits [B, V].
        """
        if not self._uses_stochastic_draft_sampling():
            raise RuntimeError("relaxed q-bonus requires stochastic draft sampling.")

        if self.method in ("eagle3", "dflash"):
            raise RuntimeError(
                "relaxed q-bonus is only implemented for draft_model/MTP."
            )
        ...
        token_ids, probs, rule_logits, _entropies = self._sample_draft_token(
            sample_hidden_states, sampling_metadata
        )
        assert probs is not None
        assert rule_logits is not None
        return token_ids, probs, rule_logits
    ...
    def propose(
        ...
        self.last_draft_entropies = None
        self.last_draft_rule_logits = None
        ...
        if self.num_speculative_tokens == 1 or self.parallel_drafting:
            (
                ...
                draft_rule_logits,
                draft_entropies,
            ) = self._sample_draft_token(sample_hidden_states, sampling_metadata)
            if draft_probs is not None:
                self.last_draft_probs = draft_probs.unsqueeze(1).contiguous()
            if draft_entropies is not None:
                self.last_draft_entropies = draft_entropies.unsqueeze(1).contiguous()
            if draft_rule_logits is not None:
                self.last_draft_rule_logits = (
                    draft_rule_logits.unsqueeze(1).contiguous()
                )
        ...
        draft_token_ids, draft_probs, draft_rule_logits, draft_entropies = (
            self._sample_draft_token(sample_hidden_states, sampling_metadata)
        )
        ...
        draft_probs_list = [draft_probs] if draft_probs is not None else []
        draft_entropies_list = (
            [draft_entropies] if draft_entropies is not None else []
        )
        draft_rule_logits_list = (
            [draft_rule_logits] if draft_rule_logits is not None else []
        )
        ...
        for token_index in range(self.num_speculative_tokens - 1):
            ...
            if draft_entropies is not None:
                draft_entropies_list.append(draft_entropies)
            if draft_rule_logits is not None:
                draft_rule_logits_list.append(draft_rule_logits)

        # [batch_size, num_speculative_tokens]
        draft_token_ids = torch.stack(draft_token_ids_list, dim=1)
        if draft_probs_list:
            self.last_draft_probs = torch.stack(draft_probs_list, dim=1).contiguous()
        if draft_entropies_list:
            self.last_draft_entropies = torch.stack(
                draft_entropies_list, dim=1
            ).contiguous()
        if draft_rule_logits_list:
            self.last_draft_rule_logits = torch.stack(
                draft_rule_logits_list, dim=1
            ).contiguous()
...
def compute_probs_and_sample_next_token(
    ...
    _validate_stochastic_draft_sampling_metadata(sampling_metadata)
    # vLLM's top-k/top-p Triton path requires float32 logits. Target-side
    # rejection sampling already casts before applying sampling constraints;
    # keep the draft-proposer path aligned with that contract.
    rule_logits = logits.to(torch.float32).contiguous()
    logits = rule_logits.clone()

    if sampling_metadata.all_greedy:
        probs = logits.softmax(dim=-1, dtype=torch.float32)
        next_token_ids = logits.argmax(dim=-1)
        entropies = entropy_from_processed_logits(logits, all_greedy=True)
        _log_mtp_draft_debug_tensors(
            "mtp_proposer_draft_sample",
            {
                "mtp_rule_logits": rule_logits,
                "mtp_processed_logits": logits,
                "mtp_probs": probs,
            },
            ...
            extra={
                ...
                "temperature": _scd_debug_tensor_to_list(
                    sampling_metadata.temperature
                ),
            },
        )
        return next_token_ids, probs, rule_logits, entropies
    ...
    entropies = entropy_from_processed_logits(
        logits,
        is_greedy=None if sampling_metadata.all_random else is_greedy,
    )
    probs = logits.softmax(dim=-1, dtype=torch.float32)
    ...
    _log_mtp_draft_debug_tensors(
        ...
        extra={
            "all_random": bool(sampling_metadata.all_random),
            "all_greedy": bool(sampling_metadata.all_greedy),
            "temperature": _scd_debug_tensor_to_list(sampling_metadata.temperature),
        },
    )
    return next_token_ids, probs, rule_logits, entropies


def _validate_stochastic_draft_sampling_metadata(
    sampling_metadata: SamplingMetadata,
) -> None:
    """Fail fast when q would not match target-side sampling processors."""
    if not sampling_metadata.no_penalties:
        raise RuntimeError(
            "Stochastic draft sampling does not support penalties yet."
        )
    if sampling_metadata.bad_words_token_ids:
        raise RuntimeError(
            "Stochastic draft sampling does not support bad words yet."
        )
    if sampling_metadata.allowed_token_ids_mask is not None:
        raise RuntimeError(
            "Stochastic draft sampling does not support allowed_token_ids yet."
        )
    active_processors = []
    for processor in sampling_metadata.logitsprocs.non_argmax_invariant:
        if isinstance(processor, MinTokensLogitsProcessor) and not processor.min_toks:
            continue
        active_processors.append(type(processor).__name__)
    if active_processors:
        raise RuntimeError(
            "Stochastic draft sampling does not support active "
            f"non-argmax invariant logits processors yet: {active_processors}."
        )
