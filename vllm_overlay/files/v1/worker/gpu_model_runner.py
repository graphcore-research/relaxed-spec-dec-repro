...
class GPUModelRunner(
    ...
    def __init__(
        ...
        self._draft_probs_req_ids: list[str] | None = None
        self._draft_entropies: torch.Tensor | None = None
        self._draft_entropies_req_ids: list[str] | None = None
        self._draft_rule_logits: torch.Tensor | None = None
        self._draft_rule_logits_req_ids: list[str] | None = None
    ...
    def _draft_probs_for_metadata(
        self, spec_decode_metadata: SpecDecodeMetadata
    ) -> torch.Tensor | None:
        """Return draft probs [sum(num_draft_tokens), V] for this target step."""
        spec_config = self.speculative_config
        needs_probs = (
            spec_config is not None
            and spec_config.method in ("draft_model", "mtp")
            and (
                spec_config.rejection_sample_method == "probabilistic"
                or spec_config.relaxed_target_method != "none"
            )
        )
        if not needs_probs:
            return None

        if self._draft_probs is None or self._draft_probs_req_ids is None:
            if sum(spec_decode_metadata.num_draft_tokens) == 0:
                return None
            raise RuntimeError(
                "Draft probabilities are required for probabilistic/relaxed "
                "draft-model/MTP rejection sampling, but none were cached."
            )

        prev_req_to_row = {
            req_id: row for row, req_id in enumerate(self._draft_probs_req_ids)
        }
        batch_size, max_draft_len, vocab_size = self._draft_probs.shape
        flat_probs = self._draft_probs.reshape(batch_size * max_draft_len, vocab_size)
        ...
        for req_idx, num_draft_tokens in enumerate(
            ...
            prev_row = prev_req_to_row.get(req_id)
            if prev_row is None:
                raise RuntimeError(f"Missing draft probabilities for request {req_id}.")
            if num_draft_tokens > max_draft_len:
                raise RuntimeError(
                    f"Scheduled {num_draft_tokens} draft tokens but cached only "
                    f"{max_draft_len} draft probability rows."
                )
            flat_indices.extend(
                prev_row * max_draft_len + pos for pos in range(num_draft_tokens)
            )

        if not flat_indices:
            return None
        index = torch.tensor(flat_indices, dtype=torch.int64, device=flat_probs.device)
        return flat_probs[index].contiguous()

    def _draft_entropies_for_metadata(
        self, spec_decode_metadata: SpecDecodeMetadata
    ) -> torch.Tensor | None:
        """Return draft entropies [sum(num_draft_tokens)] when monitored."""
        spec_config = self.speculative_config
        needs_entropies = (
            spec_config is not None
        ...
        if not needs_entropies:
            return None

        if self._draft_entropies is None or self._draft_entropies_req_ids is None:
            if sum(spec_decode_metadata.num_draft_tokens) == 0:
                return None
            if getattr(spec_config, "draft_sampling_method", "greedy") == "stochastic":
                raise RuntimeError(
                    "Draft entropies are required for entropy monitoring, "
                    "but none were cached."
                )
            return None

        prev_req_to_row = {
            req_id: row for row, req_id in enumerate(self._draft_entropies_req_ids)
        }
        batch_size, max_draft_len = self._draft_entropies.shape
        flat_entropies = self._draft_entropies.reshape(batch_size * max_draft_len)
        ...
        for req_idx, num_draft_tokens in enumerate(
            ...
            prev_row = prev_req_to_row.get(req_id)
            if prev_row is None:
                raise RuntimeError(f"Missing draft entropies for request {req_id}.")
            if num_draft_tokens > max_draft_len:
                raise RuntimeError(
                    f"Scheduled {num_draft_tokens} draft tokens but cached only "
                    f"{max_draft_len} draft entropy rows."
                )
            flat_indices.extend(
                prev_row * max_draft_len + pos for pos in range(num_draft_tokens)
            )
        ...
        if not flat_indices:
            return None
        index = torch.tensor(
            flat_indices, dtype=torch.int64, device=flat_entropies.device
        )
        return flat_entropies[index].contiguous()

    def _draft_rule_logits_for_metadata(
        self, spec_decode_metadata: SpecDecodeMetadata
    ) -> torch.Tensor | None:
        """Return draft rule logits [sum(num_draft_tokens), V] when needed."""
        spec_config = self.speculative_config
        relaxed_method = (
            None if spec_config is None else spec_config.relaxed_target_method
        )
        needs_opt_paper_logits = (
            relaxed_method == "spec_cascade_opt"
            and getattr(spec_config, "spec_cascade_opt_gate", "processed") == "paper"
        )
        needs_scd_rule_logits = relaxed_method in (
        ...
        needs_logits = (
            spec_config is not None
            and spec_config.method in ("draft_model", "mtp")
            and (needs_opt_paper_logits or needs_scd_rule_logits)
        )
        if not needs_logits:
            return None

        if self._draft_rule_logits is None or self._draft_rule_logits_req_ids is None:
            if sum(spec_decode_metadata.num_draft_tokens) == 0:
                return None
            raise RuntimeError(
                "Draft rule logits are required for this relaxed rejection "
                "sampling, but none were cached."
            )

        prev_req_to_row = {
            req_id: row for row, req_id in enumerate(self._draft_rule_logits_req_ids)
        }
        batch_size, max_draft_len, vocab_size = self._draft_rule_logits.shape
        flat_logits = self._draft_rule_logits.reshape(
            batch_size * max_draft_len, vocab_size
        )
        ...
        for req_idx, num_draft_tokens in enumerate(
            ...
            prev_row = prev_req_to_row.get(req_id)
            if prev_row is None:
                raise RuntimeError(f"Missing draft rule logits for request {req_id}.")
            if num_draft_tokens > max_draft_len:
                raise RuntimeError(
                    f"Scheduled {num_draft_tokens} draft tokens but cached only "
                    f"{max_draft_len} draft rule-logit rows."
                )
            flat_indices.extend(
                prev_row * max_draft_len + pos for pos in range(num_draft_tokens)
            )
        ...
        if not flat_indices:
            return None
        index = torch.tensor(flat_indices, dtype=torch.int64, device=flat_logits.device)
        return flat_logits[index].contiguous()
    ...
    def _sample(
        ...
        sampler_output = self.rejection_sampler(
            spec_decode_metadata,
            self._draft_probs_for_metadata(spec_decode_metadata),
            self._draft_rule_logits_for_metadata(spec_decode_metadata),
            self._draft_entropies_for_metadata(spec_decode_metadata),
            logits,
            sampling_metadata,
        )
        return sampler_output

    def _write_entropy_monitoring_records(
        self,
        req_ids: list[str],
        valid_sampled_token_ids: list[list[int]],
        entropy_stats: object,
    ) -> None:
        spec_config = self.speculative_config
        if not (
            spec_config is not None
            ...
            return
        path = getattr(spec_config, "entropy_monitoring_path", None)
        if not path or not isinstance(entropy_stats, list):
            return

        with open(path, "a", encoding="utf-8") as handle:
            for row_idx, entropy in enumerate(entropy_stats):
                if row_idx >= len(req_ids) or row_idx >= len(valid_sampled_token_ids):
                    break
                if not valid_sampled_token_ids[row_idx] or not isinstance(entropy, dict):
                    continue
                record = {"request_id": req_ids[row_idx], "entropy": entropy}
                handle.write(json.dumps(record, separators=(",", ":")))
                handle.write("\n")

    def _uses_relaxed_q_bonus(self) -> bool:
        spec_config = self.speculative_config
        return (
            spec_config is not None
            and spec_config.method in ("draft_model", "mtp")
            and spec_config.relaxed_target_method != "none"
            and spec_config.relaxed_bonus_token_policy == "relaxed_T_qp"
        )

    def _apply_relaxed_q_bonus(
        self,
        ...
        """Run the post-verification drafter pass for relaxed q-bonus."""
        if not self._uses_relaxed_q_bonus():
            return
        if spec_decode_metadata is None or common_attn_metadata is None:
            return
        if not isinstance(self.drafter, (DraftModelProposer, EagleProposer)):
            raise RuntimeError(
                "relaxed_T_qp bonus is only implemented for draft_model/MTP."
            )
        if self.speculative_config.disable_padded_drafter_batch:
            raise RuntimeError(
                "relaxed_T_qp bonus requires padded draft-model/MTP batching."
            )

        all_accepted = self.rejection_sampler.last_all_accepted_mask
        if all_accepted is None or not torch.any(all_accepted):
            return
        ...
        active_rows = torch.nonzero(
            all_accepted & (num_draft_tokens > 0), as_tuple=False
        ).flatten()
        if active_rows.numel() == 0:
            return

        q_context_token_ids = sampler_output.sampled_token_ids.clone()
        q_context_token_ids[active_rows, num_draft_tokens[active_rows]] = (
        ...
        total_num_tokens = q_bonus_attn_metadata.num_actual_tokens
        ...
        q_bonus_token_ids, q_bonus_probs, q_bonus_rule_logits = (
            self.drafter.propose_one(
                ...
                sampling_metadata=self.input_batch.sampling_metadata,
                common_attn_metadata=q_bonus_attn_metadata,
                mm_embed_inputs=mm_embed_inputs,
                num_rejected_tokens_gpu=num_rejected_tokens_gpu,
                slot_mappings=slot_mappings,
            )
        )
        self.rejection_sampler.apply_relaxed_bonus(
            sampler_output,
            spec_decode_metadata,
            q_bonus_token_ids,
            q_bonus_probs,
            self.input_batch.sampling_metadata,
            q_bonus_rule_logits=q_bonus_rule_logits,
        )
    ...
    def sample_tokens(
        ...
        def propose_draft_token_ids(sampled_token_ids):
            ...
            with record_function_or_nullcontext("gpu_model_runner: draft"):
                ...
                draft_entropies = getattr(self.drafter, "last_draft_entropies", None)
                self._draft_entropies = draft_entropies
                self._draft_entropies_req_ids = (
                    list(self.input_batch.req_ids)
                    if draft_entropies is not None
                    else None
                )
                draft_rule_logits = getattr(
                    self.drafter, "last_draft_rule_logits", None
                )
                self._draft_rule_logits = draft_rule_logits
                self._draft_rule_logits_req_ids = (
                    list(self.input_batch.req_ids)
                    if draft_rule_logits is not None
                    else None
                )
        ...
        if spec_config is not None:
            ...
            if not input_fits_in_drafter:
                # Return empty draft lists rather than dummy token ids.  The
                # scheduler will fall back to target-only decoding near the
                # drafter/context boundary, and probabilistic rejection will
                # not expect draft probability rows for fake tokens.
                ...
                self._draft_entropies = None
                self._draft_entropies_req_ids = None
                self._draft_rule_logits = None
                self._draft_rule_logits_req_ids = None
        ...
        self._write_entropy_monitoring_records(
            req_ids_output_copy,
            valid_sampled_token_ids,
            getattr(sampler_output, "entropy_stats", None),
        )
    ...
    def _dummy_sampler_run(
        ...
        if self.speculative_config:
            ...
            draft_probs = None
            draft_rule_logits = None
            if (
                self.speculative_config.rejection_sample_method == "probabilistic"
                or self.speculative_config.relaxed_target_method != "none"
            ):
                ...
                needs_opt_paper_logits = (
                    self.speculative_config.relaxed_target_method
                    == "spec_cascade_opt"
                    and getattr(
                        self.speculative_config,
                        "spec_cascade_opt_gate",
                        "processed",
                    )
                    == "paper"
                )
                needs_scd_rule_logits = (
                    self.speculative_config.relaxed_target_method
                    in ("scd_expert_toppk_gated", "scd_alpha")
                )
                if needs_opt_paper_logits or needs_scd_rule_logits:
                    draft_rule_logits = torch.randn(
                        num_tokens,
                        vocab_size,
                        device=self.device,
                        dtype=torch.float32,
